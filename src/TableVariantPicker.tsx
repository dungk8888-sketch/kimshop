...
export function TableVariantPicker({ productId, variants = [], qtyMap = {}, onQtyChange, onAddSelected, onBuySelected }: Props) {
  ...
  return <div data-kimshop-table-picker="1" className="max-w-full min-w-0 overflow-x-hidden rounded-xl border border-gray-200 bg-white p-3 space-y-3 order-3 md:order-none">
    <div className="flex items-start justify-between gap-2 min-w-0">
      <div className="min-w-0"><div className="font-bold text-sm text-gray-800">Chọn mã hàng</div><div className="text-[10px] text-gray-500 leading-snug">Mỗi mã tự đủ số lượng thì giá từng cái tự giảm.</div></div>
      <div className="text-right flex-shrink-0"><div className="text-[10px] text-gray-500">Đã chọn {picked}</div>{totals.saved>0?<><div className="text-[10px] text-gray-400 line-through">{fmt(totals.base)}</div><div className="font-bold text-[#EE4D2D] text-sm">{fmt(totals.final)}</div></>:<div className="font-bold text-[#EE4D2D] text-sm">{fmt(totals.base)}</div>}</div>
    </div>
    {promos.length>0&&<div className="rounded-lg border border-orange-100 bg-orange-50 px-3 py-2 text-[10px] text-orange-700 break-words">{promos.sort((a,b)=>Number(a.min_variant_qty||1)-Number(b.min_variant_qty||1)).map(p=>`Mua ≥ ${Math.max(1,Number(p.min_variant_qty||1))} cùng một mã: giảm ${fmt(p.discount_value)}/cái`).join(' · ')}</div>}
    <div className="relative min-w-0"><Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"/><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Nhập mã, ví dụ 11PRO / A54 / 805..." className="w-full min-w-0 border border-gray-200 rounded-lg pl-8 pr-3 py-2.5 text-xs outline-none focus:border-[#EE4D2D]"/></div>
