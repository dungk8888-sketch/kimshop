...
export default function InstallAppCard() {
  ...
  async function handleInstall() {
    ...
    catch { setError('Không mở được cửa sổ cài đặt. Hãy dùng menu của trình duyệt để thêm KIMSHOP vào màn hình chính.'); }
  }
  ...
  return <section className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4" aria-labelledby="kimshop-install-title">
    <div className="flex items-start gap-3">
      ...
      <div>
        <h3 id="kimshop-install-title" className="font-bold text-sm text-gray-800">TẢI ỨNG DỤNG KIMSHOP</h3>
        <p className="text-gray-500 text-xs mt-1">Thêm KIMSHOP vào màn hình chính để mở nhanh như một ứng dụng.</p>
      </div>
    </div>

    {installed ? <div className="flex items-center gap-2 rounded-xl bg-green-50 text-green-700 px-4 py-3 text-sm font-medium"><CheckCircle2 size={18} /> KIMSHOP đã ở trên màn hình chính của bạn.</div> : <>
      {available && <button type="button" onClick={handleInstall} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#EE4D2D] px-5 py-3 text-white text-sm font-bold hover:bg-[#d83f24] active:scale-[.99] transition w-full sm:w-auto"><Download size={17} /> Cài ứng dụng KIMSHOP</button>}
      ...
      {android && !available && <div className="bg-orange-50/70 border border-orange-100 rounded-xl p-4 text-gray-700 text-sm space-y-2">
        ...
        <p>Mở KIMSHOP bằng Chrome, nhấn menu <strong>⋮</strong> rồi chọn <strong>Cài đặt ứng dụng</strong> hoặc <strong>Thêm vào màn hình chính</strong>.</p>
      </div>}
      ...
      {!mobile && <div className="flex flex-col sm:flex-row sm:items-center gap-4 rounded-xl bg-orange-50/70 border border-orange-100 p-4">
        {qr && <img src={qr} alt="Mã QR mở KIMSHOP trên điện thoại" width="116" height="116" className="rounded-lg border border-white bg-white p-1 shrink-0" />}
        <div className="text-sm text-gray-700 space-y-1.5">
          <p className="font-semibold">Cài trên điện thoại</p>
          <p>Quét mã QR bằng điện thoại để mở KIMSHOP, rồi thêm vào màn hình chính.</p>
