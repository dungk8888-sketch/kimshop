...
const injected=`  const myPendingApplication = currentUser ? sellerApplications.find((a) => a.userId === currentUser.id && a.status === 'pending') : null;
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      const markerKey = 'kimshop_admin_order_retention_cleanup_v1';
      try {
        ...
        const ordersDel = await supabase.from('orders').delete().in('id', ids);
        if (ordersDel.error) throw ordersDel.error;
  ...
  // KIMSHOP: Admin có nút Xóa đơn ở Kênh Người Bán. Overlay độc lập để
  // không thay đổi layout/order card hiện tại đang chạy ổn.
  useEffect(() => {
    ...
    button.onclick = () => {
      ...
      note.textContent = 'Chọn đơn cần xóa. Thao tác này xóa đơn và chi tiết đơn khỏi Supabase.';
      ...
      [...orders].sort((a:any,b:any)=>new Date(b.createdAt||b.created_at||0).getTime()-new Date(a.createdAt||a.created_at||0).getTime()).forEach((o:any)=>{
      ...
      del.onclick = async () => {
        ...
        try {
          ...
          if (itemsDel.error) throw itemsDel.error;
          const orderDel = await supabase.from('orders').delete().eq('id', id);
          ...
          del.disabled = false; del.textContent = 'Xóa vĩnh viễn';
        }
      };
...
console.log('[KIMSHOP ORDERS] 2-month admin cleanup + manual admin delete applied');
