...
replaceOnce('<div className="min-h-screen bg-[#F5F5F5] font-sans text-xs text-[#333333] relative">', `<div className="min-h-screen bg-[#F5F5F5] font-sans text-xs text-[#333333] relative">
      {currentUser && !chatTarget && <button type="button" aria-label="Mở tin nhắn" onClick={() => openShopChat({})} className="fixed bottom-[calc(12rem+env(safe-area-inset-bottom))] right-4 z-[70] flex h-12 items-center justify-center gap-2 rounded-full bg-[#EE4D2D] px-4 font-semibold text-white shadow-xl hover:bg-[#df4325]"><MessageCircle size={20} /><span>Tin nhắn</span></button>}
