...
replaceOnce('<div className="min-h-screen bg-[#F5F5F5] font-sans text-xs text-[#333333] relative">', `<div className="min-h-screen bg-[#F5F5F5] font-sans text-xs text-[#333333] relative">
      {currentUser && !chatTarget && <button type="button" aria-label="Mở tin nhắn" title="Tin nhắn" onClick={() => openShopChat({})} className="fixed bottom-[9.25rem] right-3.5 z-[70] flex h-12 w-12 items-center justify-center rounded-full bg-[#EE4D2D] text-white shadow-lg shadow-orange-300/50 hover:bg-[#f63]"><MessageCircle size={21} /></button>}
